...
int main(int argc, char** argv)
{
  ...
  geometry_msgs::Quaternion quaternions[20];

  //convert the angles to quaternions
  double angle = M_PI/10;
  int angle_count = 0;
  for(angle_count = 0; angle_count < 20;angle_count++ )
  {
  ...
  geometry_msgs::Pose pose_list[20];
  int position_count = 0;
  for(position_count = 0; position_count < 20;position_count++ )
  {
      point.x = 0.5*cos((position_count+1)*M_PI/10-M_PI/2);
      point.y = 0.5*sin((position_count+1)*M_PI/10-M_PI/2)+0.5;
      point.z = 0.0;
      pose_list[position_count].position = point;
      pose_list[position_count].orientation = quaternions[1];
  }
 /* point.x = square_size;
  point.y = 0.0;
  point.z = 0.0;
